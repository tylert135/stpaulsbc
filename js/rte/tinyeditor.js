...
TINY.editor=function(){
	...
	function edit(n,obj){
		...
		var arrayLength = obj.controls.length; // array length
		var i = 0;
		...
		for(i; i<arrayLength; i++)
		{
			...
			else if(id=='font')
			{
				...
				var fonts=obj.fonts||['Verdana','Arial','Georgia'];
				var fl=fonts.length;
				var x=0;
			...
			else if(c[id])
			{
				...
				var x=c[id], func=x[2], ex, pos=x[0]*offset;
		...
		if(obj.footer)
		{
			...
			if(obj.toggle)
			{
				...
				ts.className=to.cssclass||'toggle'; ts.innerHTML=obj.toggletext||'Save';
		...
		var m='<html><head>';
		var bodyid=obj.bodyid?" id=\""+obj.bodyid+"\"":"";
		if(obj.cssfile)
		{
	...
	edit.prototype.toggle=function(post,div){
		if(!this.d){
			var v=this.t.value;
			if(div){div.innerHTML=this.obj.toggletext||'Save';}
			...
			this.IOw(this.obj.file,v);
		}
	},
	edit.prototype.getHTMLText=function(post,div){
		var v=this.e.body.innerHTML;
		v=v.replace(/<span class="apple-style-span">(.*)<\/span>/gi,'$1');
		v=v.replace(/ class="apple-style-span"/gi,'');
		v=v.replace(/<span style="">/gi,'');
